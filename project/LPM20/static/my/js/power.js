$(function(){
    ...
    $("#test").click(function () {
        ...
        $("#continuity").addClass("my-disabled");
        myUpdate();
    })
    ...
    myAjax("http://192.168.43.1:8080/readTestSetting",{},function(res){
        console.log(res);
        powerWaveRange = res.sampleTime;
    });

    // 波形图开始结束测试
    function waveSwitch(name,func){
        //startTest
    ...
    // 实时波形图的函数控制
    function waveFunc() {
        $("#wave-charts-menu a").addClass("my-disabled");
        waveSwitch("startTest",function(res){
            if(res==1){
                waveAddData();
            }else{
                alert("机器拒绝")
    ...
    //实时波形图的数据刷新
    function waveAddData() {
        myAjax("http://192.168.43.1:8080/readWaveData", {
            startTime: 0,
            endTime: powerWaveRange,
            size: 500,
        }, function (mes) {
            waveTimeout = setTimeout(waveAddData, 200);
            if (mes.status == 1) {
                wave.series[0].setData(mes.data);
            } else {
                wave.series[0].setData(mes.data);
                wave.series[1].setData([[0, mes.max], [mes.totalTime,mes.max]]);
                wave.series[2].setData([[0, mes.min], [mes.totalTime,mes.min]]);
                wave.series[3].setData([[0, mes.avg], [mes.totalTime,mes.avg]]);
                clearTimeout(waveTimeout);
                $("#test").removeClass("my-disabled");
                $("#continuity").removeClass("my-disabled");
                $("#wave-charts-menu a").removeClass("my-disabled");
            }
        });
    }
