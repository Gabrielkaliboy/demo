...
var data = makeData(0, 370.4, 10000);
...
var chart = Highcharts.chart('container', {
    ...
    // 范围选择器
    rangeSelector:{
        // allButtonEnabled: true,//是否显示所有的按钮，默认false
        buttons: [{
            // 其中 type 表示时间单位，可用的值有 millisecond、second、minute、hour、day、week、
            // month、ytd（Year to Date，最近的一年）、all（全部） ；count 为数量，text 为按钮显示的文字。
            type: 'millisecond',
            count: 1,
            text: '1毫秒'
        }, {
        ...
        buttonSpacing:10,//按钮之间的间隔
        buttonPosition:{//按钮位置
            x:0,
            y:10
        },//
        selected: 6,// 默认选中的范围，值为上面 buttons 数组的下标（从 0 开始）
        enabled:true,//是否显示范围显示器
    },
    series: [{
